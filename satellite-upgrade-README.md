...
Capsule Servers are compatible one version behind. After upgrading Satellite to 6.19, Capsules on 6.18 and 6.17 continue to work. This lets you upgrade Capsules in separate maintenance windows.
...
- [Satellite 6.19 Release Notes](https://docs.redhat.com/en/documentation/red_hat_satellite/6.19/html/release_notes/index)
...
For each version hop (e.g., 6.18 → 6.19), the included task file:

1. Enables the target version repositories via `satellite-maintain repository enable`.
2. Updates `satellite-maintain` — uses `satellite-maintain self-upgrade` for 6.19+ hops, or `dnf update rubygem-foreman_maintain` for earlier versions.
...
1. **Registry authentication** — persists `podman login` to `/root/.config/containers/auth.json`. For 6.19+, also writes to `/etc/foreman/registry-auth.json` (the new auth path used by `satellite-maintain` for container pulls).
...
> **6.19 post-upgrade note:** After upgrading to 6.19, run `insights-client --register --force` on all managed hosts to refresh their Lightspeed registration. You can use Satellite's remote execution feature to run this across your fleet.
